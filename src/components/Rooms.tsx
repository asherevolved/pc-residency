...
/* ── Fallback rooms (used while Supabase loads / if fetch fails) ── */
const fallbackRooms = [
  { id: "", name: "Deluxe A/C Room", image_url: "/images/real/room-double.jpg", price: 2225, features: ["Air Conditioning", "LED Smart TV", "Free WiFi", "Tea/Coffee"], tag: null },
  { id: "", name: "Elite A/C Room", image_url: "/images/real/room-twin.jpg", price: 1800, features: ["Air Conditioning", "Smart TV", "Tea/Coffee", "Free WiFi"], tag: "Popular" },
  { id: "", name: "Superior Non A/C Room", image_url: "/images/real/room-twin.jpg", price: 1600, features: ["Spacious Layout", "Smart TV", "Free WiFi", "Toiletries"], tag: "Best Value" },
  { id: "", name: "Superior A/C Room", image_url: "/images/real/room-double.jpg", price: 2500, features: ["Air Conditioning", "Premium Bath", "City View", "Smart TV"], tag: "Premium" },
];
...
export default function Rooms() {
  ...
  const [rooms, setRooms] = useState<RoomRow[]>(fallbackRooms);
  const [bookingRoom, setBookingRoom] = useState<RoomRow | null>(null);

  useEffect(() => {
    supabase
      .from("rooms")
      .select("id, name, image_url, price, features, tag")
      .eq("is_active", true)
      .order("price", { ascending: false })
      .then(({ data }) => {
        if (data && data.length > 0) setRooms(data as RoomRow[]);
      });
  }, []);
                ...
                <div className="p-5">
                  ...
                  <div className="mt-5 pt-4 border-t border-hairline flex items-end justify-between">
                    ...
                    <button
                      ...
                      className="w-9 h-9 rounded-full border border-charcoal/20 flex items-center justify-center text-charcoal group-hover:bg-accent group-hover:text-cream group-hover:border-accent transition-all"
